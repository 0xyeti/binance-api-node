...
var customKeepStreamAlive = function customKeepStreamAlive(url) {
  return function () {
    return fetch(url, {
      method: 'GET',
      json: true
...
var customCloseDataStream = function customCloseDataStream(url) {
  return function () {
    return fetch(url, {
      method: 'GET',
      json: true
...
var userWithListenKey = function userWithListenKey(opts) {
  return function (listenKey, keepStreamUrl, closeStreamUrl) {
    return function (cb) {
      var w = (0, _openWebsocket2.default)(BASE + '/' + listenKey);
      w.onmessage = function (msg) {
        return userEventHandler(cb)(msg);
      ...
      var int = setInterval(customKeepStreamAlive(keepStreamUrl), 50e3);
      customKeepStreamAlive(keepStreamUrl)();

      return function (options) {
        clearInterval(int);
        customCloseDataStream(closeStreamUrl);
        w.close(1000, 'Close handle was called', _extends({ keepClosed: true }, options));
      };
    };
  };
...
exports.default = function (opts) {
  return {
    ...
    user: user(opts),
    userWithListenKey: userWithListenKey(opts)
  };
};